...
//        fprintf(stderr, "Adding service '%s'\n", name);
        ...
        if ((ret = avahi_entry_group_add_service(group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, name, type, NULL, NULL, portNumber, "test=blah", r, NULL)) < 0) {
...
int main(int argc, char*argv[]) {
    ...
    while ((c = getopt (argc, argv, "?a:c:hn:p:t:v")) != -1) {
        switch(c) {
            case 'a':
                childArgs=strsave(optarg);
                break;
            case 'c':
                localClient = strsave(optarg);
                break;
            case 'h':
            case '?':
                usage();
...
//    avahi_simple_poll_loop(simple_poll);
    ...
    /* fork and exec client program here */
    ...
    if( clientPid == 0) {
        ...
        if(verbose) {
            printf("\nChild\n");
            printf("%s %s\n",localClient,childArgs);
        }
        execl(localClient,localClient,childArgs,NULL);
